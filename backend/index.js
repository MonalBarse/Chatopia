...
const { purgeOldMessages } = require('./utils/purgeMessages');
app.use(cors());
...
// Schedule the purging process (e.g., every month)
const every_month = 24 * 60 * 60 * 30; // every month
setInterval(async () => {
  try {
...
const io = require('socket.io')(server, {
  pingTimeout: 60000,
  cors: {
    origin: 'https://chatopia-f70q.onrender.com/',
  },
});
  ...
  socket.on('new message', async (newMessageRecieved) => {
    ...
    try {
      ...
      await Chat.findByIdAndUpdate(chat._id, {
        latestMessage: newMessageRecieved._id,
      });
    } catch (error) {
